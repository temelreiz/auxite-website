...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return pageMetadata('reserveVerification', locale);
}

function getCoverageStatus(ratio: number | null, t: any) {
  if (ratio === null) {
    return { class: 'coverage-initializing', badge: 'badge-info', text: t('statusInitializing'), message: t('messageInitializing') };
  }
  if (ratio >= 1.00) {
    return { class: 'coverage-success', badge: 'badge-success', text: t('statusFullyBacked'), message: t('messageFullyBacked') };
  }
  if (ratio >= 0.995) {
    return { class: 'coverage-warning', badge: 'badge-warning', text: t('statusSettlementPending'), message: t('messageSettlementPending') };
  }
  return { class: 'coverage-danger', badge: 'badge-danger', text: t('statusCoverageAlert'), message: t('messageCoverageAlert') };
}

export default async function ReserveVerificationPage() {
  const t = await getTranslations('reserveVerification');

  const metals = [
    { symbol: 'AUXG', name: t('gold'), supply: '1,000.00', allocated: '1,000.00', ratio: 1.00, unit: 'oz', color: 'var(--metal-gold)', icon: '/auxg_icon.png' },
    { symbol: 'AUXS', name: t('silver'), supply: '50,000.00', allocated: '50,000.00', ratio: 1.00, unit: 'oz', color: 'var(--metal-silver)', icon: '/auxs_icon.png' },
    { symbol: 'AUXPT', name: t('platinum'), supply: '100.00', allocated: '100.00', ratio: 1.00, unit: 'oz', color: 'var(--metal-platinum)', icon: '/auxpt_icon.png' },
    { symbol: 'AUXPD', name: t('palladium'), supply: '50.00', allocated: '50.00', ratio: 1.00, unit: 'oz', color: 'var(--metal-palladium)', icon: '/auxpd_icon.png' },
  ];

  const verificationMethods = [
    { title: t('dailyTitle'), desc: t('dailyDesc') },
    { title: t('thirdPartyTitle'), desc: t('thirdPartyDesc') },
    { title: t('vaultTitle'), desc: t('vaultDesc') },
  ];
      ...
      <section className="mobile-pt" style={{ paddingTop: '160px', paddingBottom: '80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="mobile-padding" style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ marginBottom: '64px' }}>
            ...
            <p style={{ fontSize: '18px', color: 'var(--text-muted)', maxWidth: '600px', lineHeight: 1.8 }}>
              {t('subtitle')}
            </p>
          </div>

          {/* Reserve Ratio Highlight */}
          <div style={{
            textAlign: 'center',
            padding: '48px 32px',
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              {t('reserveRatioTitle')}
            </p>
            <p style={{ fontSize: '56px', fontWeight: 700, color: 'var(--aux-gold)', margin: '0 0 8px 0', fontFamily: "'Inter', sans-serif" }}>
              {t('reserveRatioValue')}
            </p>
          ...
          <div style={{ display: 'grid', gap: '16px', marginBottom: '48px' }}>
            {verificationMethods.map((method, i) => (
              <div key={i} className="card" style={{ padding: '32px', display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                <svg style={{ width: '20px', height: '20px', color: 'var(--aux-gold)', flexShrink: 0, marginTop: '2px' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                </svg>
                <div>
                  <h3 style={{ fontFamily: "'Inter', sans-serif", fontSize: '17px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 8px 0' }}>
                    {method.title}
                  </h3>
          ...
          <div className="card" style={{ padding: '24px', marginBottom: '32px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'rgba(59, 229, 140, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              return (
                <div key={metal.symbol} className="card" style={{ padding: '24px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: `${metal.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        <Image src={metal.icon} alt={metal.name} width={28} height={28} style={{ objectFit: 'contain' }} />
                      </div>
                      <div>
                        <h3 style={{ fontFamily: "'Inter', sans-serif", fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>{metal.symbol}</h3>
